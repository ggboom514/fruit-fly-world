...
function crystalRim(ctx, f, s) {
	...
	ctx.strokeStyle = g
	// ⚠ 线宽系数必须和**炫彩幼虫**那条一致（drawLarva 里的 `l.size * 0.11`）。
	//
	//   这里原来是 0.055 —— 正好是幼虫的一半。同样的颜色、同样的流动，
	//   画出来却只是一根头发丝：用户的原话是「炫彩成虫好像没有特殊效果」，
 ...
 *   「给填充形状勾边一律不要」，炫彩那圈描边是**唯一**的例外，别当先例
 *
...
function banSheen(ctx, obj, s, buildPath) {
	...
	ctx.save()
	// ⚠ 必须自己兜住 globalAlpha：炫彩把整个身体的 alpha 设成 0.14 一直留着，
...
function drawFly(ctx, f, ox = 0, oy = 0) {
	...
	//   一律不要」，唯一的例外是炫彩，并且明写了别拿它当先例
	//
	// ⚠ 整段 save/restore 兜着，而且**显式设 globalAlpha**：
	//   炫彩在上面把 globalAlpha 设成了 0.14 留到函数末尾才还原，
	//   金蝇的闪光结尾又硬写 1（不是还原上一个值）。不自己兜住的话，
	//   「炫彩 + 疯狂」那只的光会淡到看不见
	//
	// ⚠ 石化 / 炫彩 + 疯狂时**照样画**。疯狂是行为突变，认不出来就白做了；
	...
	//      压在贴图上面才对
	//
	// ⚠ 炫彩赢：它整只是 0.14 的幽灵 + 一圈彩边，盖一层不透明的贴图上去
	//   会把它整个抹掉，两个效果同归于尽。判据和 drawLarva 里
	//   「炫彩优先于 translucency」一模一样 —— 更稀有、更该被一眼认出的那个赢
	if (nebula && !crystal) {
		flyBodyPath(ctx, s, f.sex)
	...
	// 但要在炫彩那圈彩边**之前**：炫彩是整个文件里唯一允许描边的例外，
	// 让它保持在最上面（两种效果同时有的时候，玩家先认得出的应该是炫彩）
	if (ban) banSheen(ctx, f, s, () => flyBodyPath(ctx, s, f.sex))

	// ⚠ 炫彩轮廓最后画，而且要**先把不透明度还原** ——
	//   炫彩蝇的身体是 0.14 的 alpha，描边要是也吃这个值，
	//   那圈彩虹会淡到看不见，整个炫彩就等于只有一团鬼影
	if (crystal) {
		ctx.globalAlpha = 1
		crystalRim(ctx, f, s)
	}
...
function drawLarva(ctx, l) {
	...
	// 炫彩幼虫：**优先于 translucency**。两者都是「透明」，但同时表达不出来 ——
	// 外圈微透是「体壁薄」，炫彩是「整条只剩描边」，混在一起只会两不像。
	// 炫彩更稀有、也更该被一眼认出来，所以它赢
	...
	// （星云 / 炫彩各自提前 return，剩下的走到底），每个出口都要画一次 ——
	// 一只「炫彩 + 封禁」的幼虫两条效果都该看得见。
	//
	...
	const drawBanSheen = () => {
		if (!banned) return
		// 复用**同一条** `bodyOutline()`（炫彩描边和星云贴图用的也是它），
		// 所以流光贴得严丝合缝
	...
	//
	// ⚠ 炫彩赢，理由和成虫那边一字不差：炫彩是「整条只剩一圈描边」，
	//   盖一层不透明的贴图上去就把它抹掉了
	//
	// ⚠ 这里只填贴图、**不加描边**。深紫的星云在浅色桌面上对比度本来就够，
	//   而描边会让它和炫彩幼虫长得像 —— 两者是不同突变，不该撞脸。
...
//    改挥拍 / 撒网的反馈就去改那两个调用点。
//
//  【已批准的例外】**炫彩变异**的身体轮廓（crystalRim / drawLarva 里那段）。
//    它明确违反第一条 —— 就是给幼虫和成虫的身体勾了一圈边。
//    这么做的理由：炫彩的设定就是「全身透明、只剩描边」，
...
export function drawFlyIcon(ctx, mutations, size, seed = 7, silhouette = false, now = 0) {
	...
	// ⚠ 时间**定死**。炫彩的流光色相、金蝇的闪光、疯狂的眼部呼吸都吃时间，
